...
在写入镜像前，先确认你要写入的设备。你可以用 <Man to="fdisk/8" />。插入存储设备后，用下面的命令识别设备的路径：
...
确认你要用的设备之后，确保你的设备 **没有** 被挂载，用 <Man to="umount/8" /> 卸载设备：
...
命令 <Man to="dd/1" /> 可以用来将 live 镜像拷贝到存储设备上。用 `dd`，将 live 镜像写入：
...
注意，用 CD 或 DVD 时，live 回话反应会比用 U 盘或硬盘时迟钝一点。
