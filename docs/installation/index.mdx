...
本章包括安装 Void 的一般流程。对于特殊的流程，参考[《进阶安装》](./advanced-installation-guides/index.mdx)一节。
...
注意，有些风格化安装配置要求更高的硬件配置，具体配置取决于具体的风格。

Void 不支持 i386、i486 或 i586 架构。

在安装 musl Void 前，请阅读本手册的 [musl](./musl.mdx) 一章，以获知软件的不兼容信息。
...
目前镜像都由用于发布的 siginfy 密钥签名。如果你已经在使用 Void 系统，你可以从 `void-release-keys` 软件包获取密钥，XBPS。你还需要 <Man to="signify/1" /> 或 <Man to="minisign/1" /> 的拷贝；在 Void 系统上，它们分别由 `outils` 或 `minisign` 软件包提供。
...
下面的例子展示了如何为 20210930 镜像验证 `sha256sum.txt` 文件。首先，用 `siginfy`：
...
最后，你需要检查镜像文件的校验和，于 `sha256sum.txt` 中的校验和做比较。你可以利用 <Man to="sha256/1" /> 工具，此程序依然来自 `outils` 软件包。下面演示如何验证 20210930 `x86_64` 镜像：
...
另外，如果你无法使用 `sha256` 工具，你可以计算 SHA256 哈希，比如用 <Man to="sha256sum/1" /> 计算哈希，然后用 `sha256sum.txt` 中的值比较：
