...
用 <Man to="man.conf/5" /> 配置 `man`。

`mandoc` 工具组包含了用来搜索手册页的 <Man to="apropos/1" />。可以用 <Man to="makewhatis/8" /> 命令来生成和更新 `apropos` 的数据库：
...
`mdocml` 软件包提供了每天更新 `apropos` 数据库的 cron 任务：`/etc/cron.daily/makewhatis`。要使用这个功能，你需要安装并启用 [cron 守护进程](../cron.mdx)。
...
如果使用了 `mdocml`，且设置应该应用于所有用户，那么必须将相关的路径添加到 <Man to="man.conf/5" />。例如，德语使用者会把这两行加入配置文件：
...
另外，各用户可以导出 `MANPATH` 变量到自己的环境中，详见 <Man to="man/1" />。
